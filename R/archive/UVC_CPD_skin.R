...
analyze_UVC <- function(file.name){
  ...
  ## plot qq plots to examine putative normality
  svg(paste0("Results/UVC_svg(paste0("Results/UVC_svg(paste0("Results/qqplots_",suffix, ".svg"),  width=7, height=10)
             p_qq <- dat_perc %>% ggplot(aes(sample = Count_perc)) + geom_qq() + facet_grid(Dose ~ Location, scales = "free")
             dev.off()
             
  ## calculate pvalues vs control
  pvals <- dat_perc %>% 
    group_by(Location) %>% 
    dunn_test(Count_perc~Dose, p.adjust.method = "none") %>% 
  ...
  p <-   ggboxplot(dat_perc, x = "Dose", y = "Count_perc", fill = "Type", outlier.shape = NA) +
...
<<<<<<< HEAD:R/archive/UVC_CPD_skin.R
    scale_y_continuous(guide = "prism_offset_minor", limits = c(0, 100)) + # Hier wird die y-Achse auf 0–100 % begrenzt
=======
    scale_y_continuous(guide = "prism_offset_minor", limits = c(0,100)) +
>>>>>>> dbe9d606aa352441330873a21d5ddab0dc8ec0bd:R/UVC_CPD_skin.R
